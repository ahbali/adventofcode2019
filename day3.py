...
def per_path_max_dimentions(path=[]):
    max_up = 0
    max_down = 0
    max_right = 0
    max_left = 0
    ...
    for element in path:
        ...
        if direction == "R":
            ...
            if position_horizontal > max_right:
                max_right = position_horizontal
        elif direction == "L":
            position_horizontal -= distance
            if position_horizontal < max_left:
                max_left = position_horizontal
        elif direction == "U":
            position_vertical += distance
            if position_vertical > max_up:
                max_up = position_vertical
        elif direction == "D":
            position_vertical -= distance
            if position_vertical < max_down:
                max_down = position_vertical
    return (max_left, max_right, max_down, max_up)


def global_max_dimention(max_path1=(), max_path2=()):
    result = [0, 0, 0, 0]
    for i in (0, 2):
        result[i] = min((max_path1[i], max_path2[i]))
    for i in (1, 3):
        result[i] = max((max_path1[i], max_path2[i]))
    return tuple(result)


def central_port(max_dimentions=()):
    i = -max_dimentions[0]
    j = -max_dimentions[2]
    return (i, j)


def init_grid(dimention=()):
    grid = []
    for i in range(dimention[1] - dimention[0] + 1):
        grid.append([])
        for j in range(dimention[3] - dimention[2] + 1):
            grid[i].append(0)
    return grid


def walk_path(path=[], grid=[[]], origin=(), marker=0):
    pos_x = origin[0]
    pos_y = origin[1]
    grid[pos_x][pos_y] = -1
    distences = []
    for element in path:
        ...
        if direction == "R":
            for i in range(distance):
                pos_x += 1
                old_value = grid[pos_x][pos_y]
                if old_value in (0, marker):
                    grid[pos_x][pos_y] = marker
                else:
                    grid[pos_x][pos_y] = "#"
                    distence = abs(pos_x - origin[0]) + abs(pos_y - origin[1])
            ...
            for i in range(distance):
                ...
                if old_value in (0, marker):
                    ...
                    distence = abs(pos_x - origin[0]) + abs(pos_y - origin[1])
                    distences.append(distence)
    return (grid, distences)


def min_distance(grid=[[]], origin=()):
    distences = []
    pos_x = 0
    pox_y = 0
    for i in range(len(grid)):
        for j in range(len(grid[0])):
            if grid[i][j] == "#":
                distence = abs(i - origin[0]) + abs(j - origin[1])
                distences.append(distence)
    return min(distences)


first_max = per_path_max_dimentions(first_path)
second_max = per_path_max_dimentions(second_path)
dimention = global_max_dimention(first_max, second_max)
origin = central_port(dimention)
grid = init_grid(dimention)
min_dist = []
grid, min_dist = walk_path(first_path, grid, origin, 1)
grid, min_dist = walk_path(second_path, grid, origin, 2)

# min_intersection = min_distance(grid, origin)

print("first path max's = ", first_max)
print("second path max's = ", second_max)
print("global max dimentions = ", dimention)
print("central port = ", origin)
# print("grid = ", grid)
print("min distance intersection = ", min(min_dist))
