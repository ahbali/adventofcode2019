...
def path_to_set_coordinates(path=[]):
    ...
    for element in path:
        ...
        if direction == "R":
            for i in range(distance):
                pos_x += 1
                path_coordinates.add((pos_x, pos_y))
        elif direction == "L":
            for i in range(distance):
                pos_x -= 1
                path_coordinates.add((pos_x, pos_y))
...
def distances_to_intersections(path=[], inter=set()):
    intersections = inter.copy()
    pos_x = 0
    pos_y = 0
    steps = 0
    distances = {}
    ...
    for element in path:
        ...
        if direction == "R":
            for i in range(distance):
                pos_x += 1
                steps += 1
                if (pos_x, pos_y) in intersections:
                    intersections.remove((pos_x, pos_y))
                    distances[(pos_x, pos_y)] = steps
...
def min_distance(distances1={}, distances2={}):
    infinity = float("inf")
    min_dist = infinity
    for element in distances1:
        temp = distances1.get(element) + distances2.get(element)
        if temp < min_dist:
            min_dist = temp
    return min_dist


firt_path_set = path_to_set_coordinates(first_path)
second_path_set = path_to_set_coordinates(second_path)

paths_intersections = firt_path_set & second_path_set

firt_path_dist_dict = distances_to_intersections(first_path,
                                                 paths_intersections)
second_path_dist_dict = distances_to_intersections(second_path,
                                                   paths_intersections)
min_dist = min_distance(firt_path_dist_dict, second_path_dist_dict)
